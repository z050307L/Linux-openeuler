...
## 二、openEuler 替换国内镜像源（华为云 / 阿里云镜像）
openEuler 默认官方源国外访问慢，国内主流镜像：华为镜像站、阿里云镜像站。


1. 查看系统版本

首先，确认当前 openEuler 系统的版本，以选择适配的镜像源：
```bash
cat /etc/*-release
```
...
[OS]
name=OS
baseurl=https://mirrors.163.com/openeuler/openEuler-24.03-LTS-SP4/OS/$basearch/
enabled=1
gpgcheck=1
gpgkey=https://mirrors.163.com/openeuler/openEuler-24.03-LTS-SP4/OS/$basearch/RPM-GPG-KEY-openEuler
[everything]
name=everything
baseurl=https://mirrors.163.com/openeuler/openEuler-24.03-LTS-SP4/everything/$basearch/
enabled=1
gpgcheck=1
gpgkey=https://mirrors.163.com/openeuler/openEuler-24.03-LTS-SP4/everything/$basearch/RPM-GPG-KEY-openEuler
[update]
name=update
baseurl=https://mirrors.163.com/openeuler/openEuler-24.03-LTS-SP4/update/$basearch/
enabled=1
gpgcheck=1
gpgkey=https://mirrors.163.com/openeuler/openEuler-24.03-LTS-SP4/OS/$basearch/RPM-GPG-KEY-openEuler
```
...
可以参考CentOS Everything。


CentOS Everything 版本 是 CentOS 提供的一种 ISO 镜像类型，主要特点是包含了完整的软件包集合。它适用于需要全面软件支持的用户，尤其是在需要本地镜像或离线安装的场景中。

特点与用途：

Everything 版本的主要功能是对标准安装盘（DVD ISO）的软件进行补充，集成了 CentOS 官方提供的所有软件包。这使得它成为一个完整的安装盘，可以用来：

  - 安装系统时提供更多的软件选择。

  - 构建本地软件包镜像，方便离线环境使用。

有了CentOS Everything之后，可以直接从里面打包所需软件的安装包。